...
// 변수지정을 해줬다.
const apiKey =
    '9wZ%2BrDU8Rw9aGoABTm8p6z3qEOHzGQIcxScp0YEj%2FdRlKNreVbpR2QNFbYWB%2F%2B6CGT3PhJKUMclAZ59a5JV2mA%3D%3D';
...
class WeatherModel {
  ...
  Future<dynamic> getLocationWeather() async {
    ...
    void currentWeatherDate() {
      ...
      if (now.minute <= 40) {
        ...
        if (now.hour == 0) {
          ...
          currentBaseTime =
              DateFormat('HH00').format(now.subtract(const Duration(hours: 1)));
        }
      }
    ...
    void superShortWeatherDate() {
      ...
      if (now.minute <= 45) {
        ...
        if (now.hour == 0) {
          ...
          sswBaseTime =
              DateFormat('HH30').format(now.subtract(const Duration(hours: 1)));
        }
      }
    ...
    Network network = Network(today2am, shortTermWeather, currentWeather,
